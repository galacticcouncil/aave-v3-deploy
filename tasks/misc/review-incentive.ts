...
task(`review-incentive`, ``)
  ...
  .setAction(
    async (
      { batch, reserve, update }: { batch: boolean, reserve: string, update: boolean },
      hre
  ) => {
  ...
  const onChainIncentives = await inc.getReservesIncentivesData(poolAddressProvider.address);
  ...
  if (!chainlinkConf) {
    console.log(chalk.red(`'${network}.${reserve}': chainlink configuration not found`));
    exit(1);
  }
  ...
  for (let i = 0; i < incentiveConf.length; i++) {
    ...
    if (!oracleAddr || oracleAddr == ZERO_ADDRESS ) {
      console.log(chalk.red(`'${network}.${reserve}[${i}]': oracle wasn't found in ChainlinkAggregator or is not valid`));
      exit(1);
    }

    if (cfg.transferStrategy != TransferStrategy.PotRewardsStrategy) {
      console.log(chalk.red(`'${network}.${reserve}[${i}]': invalid transfer strategy. Only PotRewardsStrategy is supported`));
      exit(1);
    }

    if (!cfg.reward || cfg.reward == ZERO_ADDRESS) {
      console.log(chalk.red(`'${network}.${reserve}[${i}]': invalid reward address`));
      exit(1);
    }

    if ( cfg.emissionAdmin.toLowerCase() != admin.toLowerCase()) {
      console.log(chalk.red(`'${network}.${reserve}[${i}]': emission admin is not set to poolAdmin`));
      exit(1);
    }

    const onChainInc = onChainIncentives.find((el) => el.underlyingAsset == reserveAddr);
    if (!onChainInc) {
      //NOTE: onChainInc should always exists even if incentives were never deployed
      console.log(chalk.red(`'${network}.${reserve}[${i}]': unexpected error. No incentives data found onchain.`));
      exit(1);
    }
    ...
    switch (cfg.incentivizedToken) {
      ...
      case AssetType.VariableDebtToken:
        activeInc = onChainInc.vIncentiveData.rewardsTokenInformation.find(el => el.rewardTokenAddress == cfg.reward );
        asset = variableDebtTokenAddress;
        break;
      case AssetType.StableDebtToken:
        activeInc = onChainInc.sIncentiveData.rewardsTokenInformation.find(el => el.rewardTokenAddress == cfg.reward );
    ...
    if (activeInc?.emissionEndTimestamp.gt(BigNumber.from(now)) && !update) {
      console.log(chalk.yellow(`'${network}.${reserve}[${i}]': liquidity mining is active. To update it use --update`));
      continue;
    }
  ...
  if (assetsConf.length != 0) {
    const tx = await em.populateTransaction.configureAssets(assetsConf, { gasLimit: 1000000 });
    addTransaction(tx);
  }
