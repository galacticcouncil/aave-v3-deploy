...
- **HDCLOracleAdapter** — `aave-v3-deploy/contracts/HDCLOracleAdapter.sol`.
  ...
  AaveOracle as the price source for the DCL reserve) **and** the modern
  ...
  re-deployed and the existing HDCL pool's AaveOracle is re-pointed via the
  stablepool launch proposal (so there's only one source of truth).
- **HDCLDepositZap** — `aave-v3-deploy/contracts/HDCLDepositZap.sol`.
  ...
  holding HDCL aToken" flow without leaving dust in the user's wallet on
...
it adds the HDCL/HOLLAR stableswap pool + Treasury liquidity bootstrap,
moves HDCLDepositZap deploy earlier (before proposal generation), and
...
13. **Deploy HDCLDepositZap** — once governance has registered HDCL as Erc20
    and whitelisted Pool-Proxy-HDCL in `EVMAccounts.ApprovedContract` (both
    happen in Phase D of the proposal), deploy the zap helper:
    ```bash
    HARDHAT_NETWORK=<network> MARKET_NAME=HDCL npx hardhat deploy-HDCLDepositZap \
      --hollar     <HOLLAR address> \
      --vault      <vault proxy address> \
      --pool       <Pool-Proxy-HDCL address> \
...
14. **UI update** — bump vault/pool/oracle/**zap** addresses in
    `hydration-ui/apps/main/src/modules/hdcl-vault/constants.ts`
    (`HDCL_DEPOSIT_ZAP_ADDRESS` is what the UI's `useDeposit` calls
...
Lark launched in stages — HDCL pool first, then HDCLDepositZap separately,
...
governance config, the deposit zap, the HDCL/HOLLAR stableswap pool, AND
...
> The lark task `tasks/proposals/hdcl.ts` is the staged version. Mainnet
> uses a NEW task `tasks/proposals/hdcl-mainnet-launch.ts` (to be written,
> mirroring `heurc-launch.ts` structure) which composes the full
> single-batch shape: the existing HDCL governance contents PLUS everything
> below.

### New components

#### `2-Pool-HDCL` — stableswap LP token (asset 10055)

Convention follows HEURC's 10044 (`10000 + reserve-symbol-numeric-id`).
Registered alongside the existing DCL/HDCL registrations in Phase D, as a
...
#### Stableswap pool: HDCL ↔ HOLLAR

Created via `stableswap.createPoolWithPegs(...)`:

| Field | Value | Reasoning |
|---|---|---|
| `shareAsset` | 10055 | The 2-Pool-HDCL LP token registered above |
| `assets` | `[55, 222]` | **Sorted ascending** — HDCL(55) before HOLLAR(222). Inversion silently produces wrong pool composition. |
...
| `pegSource[0]` | `{ MMOracle: <HDCLOracleAdapter address> }` | HDCL's peg source. **Reuses the same oracle deployed for the Aave reserve** — wraps `vault.exchangeRate()` and scaled 18→8 decimals. **Must implement Chainlink V3 `AggregatorV3Interface`** (`latestRoundData`, etc.) — the stableswap pallet's MMOracle resolver calls `latestRoundData()`, not the legacy `latestAnswer()`. Lesson 11 captures the lark debugging history; the current `HDCLOracleAdapter.sol` in this repo has both interfaces and works for both consumers. |
| `pegSource[1]` | `{ value: [1, 1] }` | HOLLAR fixed 1:1 base reference |
| `maxPegUpdate` | 200 | Gigasol's "≥10× expected APY" rule applied to HDCL's ~18% APY = floor of 180, rounded to 200. Sits in the same magnitude as gigasol(160 for 7% APY) and prime(120) — both yield-bearing rebase tokens. ~4× margin over HDCL's natural daily drift (~0.05%/day): tight enough to bound oracle manipulation, loose enough to never throttle yield accrual. **Caveat: if you can find someone on the chain team who knows the pallet definition of this unit, worth confirming before locking in.** |
...
HDCL ecosystem, the router needs an explicit hop chain registered via
`router.forceInsertRoute`. Without these routes, users CAN still hold
HDCL/aHDCL and use the pool, but **they can't pay fees in H2O / WETH
while interacting with HDCL surfaces** — the router has no path from
...
route per fee-currency source, ending at the **user-held asset** (HDCL =
the aToken — asset 55 on mainnet, asset 550 on lark), with the new
stableswap as a hop.

For mainnet (HDCL = asset 55 after the new naming flip):

| `assetIn` → `assetOut` | Route hops |
|---|---|
| `H2O (1) → HDCL (55)` | `[Omnipool: 1→222, Stableswap 10055: 222→55]` |
| `WETH (20) → HDCL (55)` | `[Stableswap: 104, 20→1007], [Stableswap: 4200, 1007→4200], [Aave: 4200→420], [Omnipool: 420→222], [Stableswap: 10055, 222→55]` |
...
2. **Approve HDCLDepositZap for 300K HOLLAR.**
   ```
   HOLLAR.approve(HDCLDepositZap, 300000e18)
   ```
   Single-shot allowance.

3. **Deposit 300K HOLLAR via the zap → mint ≈300K HDCL aToken atomic.**
   ```
   zap.depositAndSupply(300000e18)
   ```
   Produces ≈300K HDCL (asset 55) in Treasury's wallet. Atomic at the EVM
...
4. **Add liquidity to the new pool.**
   ...
   stableswap.addAssetsLiquidity({
     poolId: 10055,
     assets: [
       { assetId: 55,  amount: <actual HDCL minted> },  // HDCL first
...
> (a) hardcode `300000e18` and accept that a tiny dust of HDCL stays in
> treasury's wallet (acceptable but not ideal), or (b) wrap step 3+4 in a
> small "addAllAvailableHdcl" helper precompile that reads the actual
...
| 9 (proposal gen) | **Use `tasks/proposals/hdcl-mainnet-launch.ts`** — to be written, mirrors `heurc-launch.ts` structure, composes the full single-batch shape (existing HDCL governance contents + 2-Pool-HDCL registration + stableswap creation + Treasury bootstrap). The lark task `hdcl.ts` is the staged version and is NOT used on mainnet. |
...
| **13 (zap deploy) — MOVES UP** | **The HDCLDepositZap must be deployed BEFORE Phase 9** (proposal generation). The mainnet proposal calls `zap.depositAndSupply` from Treasury in step 3 of the bootstrap, so the zap address must be fixed at proposal-build time. Run the existing `deploy-HDCLDepositZap` task immediately after Phase 7 (admin transfer); use the *mainnet* vault address. |
| 14 (UI bump) | **Unchanged**, plus stablepool address (10055) goes into `constants.ts` alongside the others. UI Phase 5 (instant redeem) is now implemented on lark — only the `STABLESWAP_HDCL_ASSET_ID` constant needs flipping from `550n` (lark naming) to `55n` (mainnet naming after the asset rename). See `hydration-ui/HDCL-UI-HANDOVER.md` "Phase 5 implementation" for the file-level summary. |

### Pre-flight (additions to the existing checklist)

```bash
# HDCLDepositZap deployed and constructor max-approve completed
cast call HDCLDepositZap "HOLLAR()(address)"        # → HOLLAR mainnet address
cast call HDCLDepositZap "VAULT()(address)"         # → mainnet vault proxy
cast call HOLLAR "allowance(address,address)(uint256)" \
  HDCLDepositZap MAINNET_VAULT_ADDR                 # → 2^256 − 1
...
# HDCLOracleAdapter is reading the right vault and returning sane price
cast call HDCLOracleAdapter "vault()(address)"           # → mainnet vault proxy
cast call HDCLOracleAdapter "latestRoundData()(...)"     # answer ≈ 1.0e8 at launch (no yield accrued yet)

# Confirm 2-Pool-HDCL asset id is free
polkadot-api:
...
Run AFTER governance executes, in addition to the existing HDCL-side
checks (HDCL aToken proxy non-zero, etc.):

```bash
# Stableswap pool exists with the right shape
polkadot-api:
  assetRegistry.assets(10055)              # name="2-Pool-HDCL", type=StableSwap
...
# Pool composition matches what we sent
  tokens.accounts(stableswap-pool-account-addr, 55)   # ≈ 300_000e18 HDCL
  tokens.accounts(stableswap-pool-account-addr, 222)  # ≈ 300_000e18 HOLLAR

# Spot-quote sanity: 1 HDCL → ≈ 1 HOLLAR (within fee + minor amp curvature)
# via router or direct stableswap call — confirms peg / liquidity is healthy
```

### Lark stablepool dry-run — verified 2026-05-01

`tasks/proposals/hdcl-stablepool-lark.ts` was dry-run on a chopsticks fork
...
=== Post-state ===
  assetRegistry.assets(10055):                       2-Pool-HDCL, StableSwap, 18 dec ✓
  ...
  treasury aHDCL (550, via CurrenciesApi):           896.56 (= safety-buffer dust) ✓
...
storage) returns 0 for **Erc20-typed** assets (HOLLAR=222, aHDCL=550) —
...
- **Asset-sort order in `addAssetsLiquidity`.** Stableswap enforces
  ascending asset-id sort. HDCL=55 < HOLLAR=222, so the array must be
  `[HDCL, HOLLAR]`. Inverted order silently produces wrong pool
  composition. Caught on dry-run by inspecting `tokens.accounts(pool, _)`.
- **`HDCLOracleAdapter.vault()` mismatch.** If the oracle was deployed
  ...
  do NOT skip.
- **`maxPegUpdate=200` too tight if our APY estimate is wrong.** If HDCL
...
the original HDCL launch (handover lesson 10). Renaming on lark via
`assetRegistry.update` is risky because the live Aave HDCL pool was
...
|---|---|---|
| 55 | `HDCL` → `0xb82c…548D8` (vault proxy) | `HDCL` → aToken proxy (the user-held token) |
| 550 | `aHDCL` → `0x9cd4…ada2` (aToken proxy) | `DCL` → vault proxy (underlying) |

**Decision:** leave lark's asset registry alone. The stablepool launch task
(`hdcl-stablepool-lark.ts`) uses **asset 550** as the HDCL-aToken side
because that's what users actually hold on lark. The mainnet variant
(`hdcl-mainnet-launch.ts`) will use **asset 55** because mainnet starts
in the new naming.
...
**Sort order on lark:** ascending → `[HOLLAR=222, aHDCL=550]`. Pegs follow
the same order: HOLLAR fixed `[1,1]` first, aHDCL `MMOracle` second.

**Sort order on mainnet:** ascending → `[HDCL=55, HOLLAR=222]`. Pegs:
HDCL `MMOracle` first, HOLLAR fixed `[1,1]` second. The previous
...
| **Launch shape** | **Staged: HDCL pool first, zap separately, stableswap deferred** | **Single-batch: vault + pool + governance + zap + stableswap + Treasury bootstrap, all in one referendum.** See "Mainnet single-batch launch composition" above. |
| **Stableswap pool** | Not deployed (still unbuilt as of writing) | 2-Pool-HDCL (10055), HDCL/HOLLAR pair, amp=100, fee=0.1%, peg via existing HDCLOracleAdapter, maxPegUpdate=200 |
| **Initial liquidity** | None | Treasury borrows 600K HOLLAR from main MM; pairs 300K HOLLAR + 300K HDCL into the new pool; resulting LP shares stay in Treasury as protocol-owned liquidity |
...
`pool.supply(precompile, predictedHdcl, user, 0)`, where `predictedHdcl` came
...
1. **Drift between `previewDeposit` read and `vault.deposit` execution.** Yield
   ...
   out a few wei smaller than `predictedHdcl`. `pool.supply(predictedHdcl)`
...
2. **`Utility.batch_all` is atomic at the substrate dispatch level, not at the
   ...
   user with `vault.deposit` committed (HDCL minted to wallet) and
   `pool.supply` reverted (no aHDCL minted). This is the same trap as
...
We tried percentage-based safety buffers on `predictedHdcl` (subtract 0.001%
to absorb drift) but the dust scaled with deposit size: a 222k HOLLAR deposit
left ~2.2 HDCL behind per batch, which adds up.

**Fix:** `HDCLDepositZap.sol` (lesson moved into the Components section
...
Without routes from H2O/WETH to HDCL, users can hold and trade HDCL
fine, but **they can't pay fees in H2O / WETH while interacting with
HDCL surfaces** — the conversion step has no path.
...
I missed this in `hdcl-stablepool-lark.ts` and shipped a separate
follow-up batch (`scripts/submit-hdcl-routes-lark.mjs`) with two routes:
`H2O→HDCL` and `WETH→HDCL`. The mainnet launch should bundle the
...
Routes between assets that are already in the same pool (e.g. HOLLAR↔HDCL)
...
### 11. HDCLOracleAdapter needed Chainlink V3 interface for stableswap MMOracle (cost: one chopsticks dry-run + a re-deploy on lark)

The original `HDCLOracleAdapter.sol` implemented only Aave's
...
calls, so the HDCL Aave pool worked fine.
...
I caught this on the first chopsticks dry-run of `hdcl-stablepool-lark.ts`:
...
HDCLOracleAdapter (failing) showed the working oracle returns valid data
for `latestRoundData()` while ours reverted.

**Fix:** added the full Chainlink V3 surface to HDCLOracleAdapter
...
call to re-point the HDCL pool's AaveOracle source to the new oracle —
so lark ends up with one consolidated oracle instead of two.

**For mainnet:** deploy the V3-compliant HDCLOracleAdapter from the start.
...
| HDCLOracleAdapter (legacy, V1 — used by HDCL pool's AaveOracle until re-pointed by the stablepool launch proposal) | `0x19Cb1536947bA792d71c04F4dBa9DcDF63C840A7` |
| HDCLOracleAdapter (V3 — re-deployed 2026-05-01 with Chainlink V3 interface; used by stablepool MMOracle peg AND consolidated as the HDCL pool's source) | `0x90a6B6357bA5925e730657813173C4B1a9af8346` |
| HDCLDepositZap | `0x75d09AbAF2005b0ba06abE6a49796D0180D9a375` |
...
- `tasks/misc/deploy-HDCLOracleAdapter.ts` — Phase 2.
- `tasks/misc/deploy-HDCLDepositZap.ts` — On mainnet runs **before** proposal generation (so the proposal can reference the zap address). On lark this was Phase 13 (post-execution). Atomic deposit+supply helper.
- `tasks/proposals/hdcl.ts` — **Lark only.** First-stage launch: HDCL pool + governance config. Already executed on lark.
- `tasks/proposals/hdcl-stablepool-lark.ts` — **Lark only.** Second-stage launch: 2-Pool-HDCL asset + stableswap pool + Treasury liquidity bootstrap (full borrow + deposit + add-liquidity flow). Run AFTER `hdcl.ts` and AFTER `HDCLDepositZap` is deployed. Submitted via Root track (non-urgent). Treasury bootstrap uses `dispatchAs(treasury, evm.call(...))` for the borrow + approve + zap.depositAndSupply EVM calls — this pattern is unprecedented in this repo (heurc/giga* all use substrate-only Treasury calls), so dry-run carefully on chopsticks before lark submission. **Uses asset 550 (`aHDCL` under lark's old naming) for the stablepool's HDCL side**, NOT asset 55 — see "Lark vs mainnet asset id divergence" below.
- `tasks/proposals/hdcl-mainnet-launch.ts` — **Mainnet only. To be written**, mirroring `heurc-launch.ts`. Composes the full single-batch shape per "Mainnet single-batch launch composition" above. Combines what `hdcl.ts` and `hdcl-stablepool-lark.ts` do across two stages on lark, plus the Treasury borrow step.
- `submit-hdcl-proposal.ts` — generates + submits the **original** HDCL launch proposal (vault + Aave pool + governance config). *Currently configured for lark Root-track; for mainnet, switch back to the WhitelistedCaller flow (see "Governance submission" above) AND point at `hdcl-mainnet-launch` task.*
- `submit-hdcl-stablepool-lark.ts` — submits the lark stablepool launch proposal (`hdcl-stablepool-lark` task) end-to-end. Defaults to chopsticks (`PROPOSAL_WS=ws://localhost:8000`); set `PROPOSAL_WS=wss://0.lark.hydration.cloud` to run against live lark. Reads HDCL state via `RPC=https://0.lark.hydration.cloud` for the proposal build (chopsticks doesn't expose `eth_call`/`net_version`, so the build step needs the live lark RPC; submission still goes to chopsticks for the dry-run).
- `submit-hdcl-routes-lark.mjs` — **lark-only follow-up.** Builds + submits a Root proposal that registers `router.forceInsertRoute` calls for `H2O→HDCL(550)` and `WETH→HDCL(550)` (lesson 12). Standalone @polkadot/api script, no hardhat. **NOT needed on mainnet** — the route inserts will be in the unified `hdcl-mainnet-launch.ts` proposal task per "Router routes" in the launch composition section.
