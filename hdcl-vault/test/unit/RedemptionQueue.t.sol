...
contract RedemptionQueueTest is BaseTest {
    ...
    function test_requestRedeem_escrowsHdcl() public {
        ...
        uint256 redeemAmount = hdcl / 2;
        _requestRedeem(alice, redeemAmount);

        // HDCL transferred from Alice to vault (escrowed)
        assertEq(vault.balanceOf(alice), aliceBalBefore - redeemAmount, "Alice HDCL should decrease");
        assertEq(
            vault.balanceOf(address(vault)),
            vaultBalBefore + redeemAmount,
            "Vault HDCL balance should increase (escrow)"
        );
    }
    ...
    function test_requestRedeem_updatesTotalQueued() public {
        ...
        assertEq(vault.totalQueuedHdcl(), 0, "totalQueuedHdcl should start at 0");

        uint256 redeemAmount = hdcl / 2;
        _requestRedeem(alice, redeemAmount);

        assertEq(vault.totalQueuedHdcl(), redeemAmount, "totalQueuedHdcl should increase");
    }
    ...
    function test_cancelRedeem_returnsHdcl() public {
        uint256 hdcl = _deposit(alice, TEN_THOUSAND_HOLLAR);
        uint256 redeemAmount = hdcl / 2;
        ...
        // HDCL should be returned to Alice
        assertEq(
            vault.balanceOf(alice),
            aliceBalBefore + redeemAmount,
            "Alice should get HDCL back after cancel"
        );

        // totalQueuedHdcl should decrease
    ...
    function test_processQueue_fullyFulfills() public {
        // 1. Alice deposits 10,000 HOLLAR -> gets HDCL
        ...
        // 3. Alice requests redeem of a small portion of HDCL
        uint256 redeemAmount = aliceHdcl / 4;
        _requestRedeem(alice, redeemAmount);
    ...
    function test_processQueue_partialFulfillment() public {
        ...
        // 3. Alice requests redeem of ALL her HDCL (this requires more HOLLAR than idle might have
        ...
        // Bob also deposits, warp and process so we have more HDCL
        _deposit(bob, TEN_THOUSAND_HOLLAR);
        _warpDays(61);
        _processPositionFull(1);

        // Request a very large redeem: all of Alice's + Bob's HDCL
        ...
        // Use only one position worth of idle, but queue both users' HDCL.
    ...
    function test_processQueue_partialFulfillment_focused() public {
        ...
        // Request all of Alice's HDCL as redeem
        _requestRedeem(alice, aliceHdcl);
        ...
        uint256 totalQueued = vault.totalQueuedHdcl();
    ...
    function test_processQueue_multipleRequests() public {
        ...
        uint256 aliceRedeem = aliceHdcl / 4;
        uint256 bobRedeem = bobHdcl / 4;
    ...
    function test_processQueue_skipsInactiveEntries() public {
        ...
        uint256 request0 = _requestRedeem(alice, aliceHdcl / 4);
        uint256 request1 = _requestRedeem(alice, aliceHdcl / 4);
    ...
    function test_processQueue_burnAtCurrentRate() public {
        ...
        assertApproxEqRel(
            ...
            expectedHollar,
            0.01e18,
            "HOLLAR received should match HDCL * rate"
        );
    }
    ...
    function test_processQueue_preservesExchangeRate() public {
        ...
        // Bob requests redeem of half his HDCL
