...
const func = async function ({ getNamedAccounts, deployments, ...hre }) {
    ...
    if (isPoolProxyPending) {
        ...
        deployments.log(`[Deployment] Attached Pool implementation and deployed proxy contract: ${txPoolProxyAddress}`);
    ...
    await (0, tx_1.waitForTx)(await poolConfiguratorInstance.updateFlashloanPremiumTotal(poolConfig.FlashLoanPremiums.total, { gasLimit: 500000 }));
    // Set protocol Flash Loan Premium
    console.log("Setting Flash Loan Premiums to Protocol");
    await (0, tx_1.waitForTx)(await poolConfiguratorInstance.updateFlashloanPremiumToProtocol(poolConfig.FlashLoanPremiums.protocol, { gasLimit: 500000 }));
    return true;
};
