...
const func = async function ({ getNamedAccounts, deployments, ...hre }) {
    ...
    if (!paraswapAugustusRegistry) {
        console.log("[WARNING] Skipping the deployment of the Paraswap adapters due missing 'ParaswapRegistry' address at pool configuration.");
        return;
    }
    const { address: addressesProvider } = await deployments.get(helpers_1.POOL_ADDRESSES_PROVIDER_ID);
    const poolAdmin = helpers_1.GOVERNANCE_BRIDGE_EXECUTOR[network] || helpers_1.POOL_ADMIN[network];
