...
(0, config_1.task)(`review-borrow-caps`, ``)
    // --fix
    .addFlag("fix")
    .addFlag("batch")
    ...
    .setAction(async ({ fix, checkOnly, batch, }, hre) => {
    ...
    const dataProvider = await (0, contract_getters_1.getAaveProtocolDataProvider)(await (0, tx_1.getAddressFromJson)(network, deploy_ids_1.POOL_DATA_PROVIDER));
    ...
    for (let index = 0; index < reservesToCheck.length; index++) {
        ...
        const expectedBorrowCap = poolConfig.ReservesConfig[normalizedSymbol.toUpperCase()].borrowCap;
        const onChainBorrowCap = (await dataProvider.getReserveCaps(tokenAddress)).borrowCap.toString();
        const delta = expectedBorrowCap !== onChainBorrowCap;
        if (delta) {
            console.log("- Found differences of the borrow cap for ", normalizedSymbol);
            console.log("  - Expected:", Number(expectedBorrowCap).toLocaleString(undefined, {
                currency: "usd",
            }));
            console.log("  - Current :", Number(onChainBorrowCap).toLocaleString(undefined, {
                currency: "usd",
            }));
            if (!fix) {
                continue;
            ...
            const tx = await poolConfigurator.populateTransaction.setBorrowCap(tokenAddress, expectedBorrowCap, { gasLimit: 100000 });
            if (batch) {
                (0, transaction_batch_1.addTransaction)(tx);
            }
            else {
                await (0, tx_1.waitForTx)(await poolConfigurator.signer.sendTransaction(tx));
                const newOnChainBorrowCap = (await dataProvider.getReserveCaps(tokenAddress)).borrowCap.toString();
                console.log("[FIX] Set ", normalizedSymbol, "Borrow cap to", Number(newOnChainBorrowCap).toLocaleString(undefined, {
                    currency: "usd",
                }));
            }
        }
        else {
            console.log(chalk_1.default.green(`  - Reserve ${normalizedSymbol} borrow cap follows the expected configuration`));
        }
    }
});
