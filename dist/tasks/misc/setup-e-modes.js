...
(0, config_1.task)(`setup-e-modes`, `Setups e-modes from config`).setAction(async (_, hre) => {
    ...
    for (let key in config.EModes) {
        ...
        await (0, tx_1.waitForTx)(await poolConfigurator.setEModeCategory(eMode.id, eMode.ltv, eMode.liquidationThreshold, eMode.liquidationBonus, oracle, eMode.label, { gasLimit: 100000 }));
