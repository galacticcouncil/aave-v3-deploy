    ...
    for (let index = 0; index < reservesToCheck.length; index++) {
        ...
        if (delta) {
            ...
            if (process.env.ENCODE_ONLY) {
                const tx = await poolConfigurator.populateTransaction.setReserveStableRateBorrowing(tokenAddress, expectedStableRateEnabled);
                console.log(tx);
            }
            else {
                await (0, tx_1.waitForTx)(await poolConfigurator.setReserveStableRateBorrowing(tokenAddress, expectedStableRateEnabled, { gasLimit: 100000 }));
            }
