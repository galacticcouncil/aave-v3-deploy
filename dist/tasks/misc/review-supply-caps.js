...
(0, config_1.task)(`review-supply-caps`, ``)
    ...
    for (let index = 0; index < reservesToCheck.length; index++) {
        ...
        if (delta) {
            ...
            const tx = await poolConfigurator.populateTransaction.setSupplyCap(tokenAddress, expectedSupplyCap, { gasLimit: 100000 });
            if (batch) {
                (0, transaction_batch_1.addTransaction)(tx);
            }
            else {
                await (0, tx_2.waitForTx)(await poolConfigurator.signer.sendTransaction(tx));
