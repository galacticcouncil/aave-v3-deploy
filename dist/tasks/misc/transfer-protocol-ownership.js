...
(0, config_1.task)(`transfer-protocol-ownership`, `Transfer the ownership of protocol from deployer`).setAction(async (_, hre) => {
    ...
    if (!desiredAdmin) {
        console.error("The constant desired Multisig is undefined. Check missing admin address at MULTISIG_ADDRESS or GOVERNANCE_BRIDGE_EXECUTOR constant for the network", networkId);
    ...
    if (process.env.ENCODE_ONLY) {
        const add = await aclManager.populateTransaction.addEmergencyAdmin(desiredAdmin);
        const remove = await aclManager.populateTransaction.removeEmergencyAdmin(emergencyAdmin);
