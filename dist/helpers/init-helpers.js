...
const initReservesByHelper = async (reservesParams, tokenAddresses, aTokenNamePrefix, stableDebtTokenNamePrefix, variableDebtTokenNamePrefix, symbolPrefix, admin, treasuryAddress, incentivesController, batch = false) => {
    ...
    for (let chunkIndex = 0; chunkIndex < chunkedInitInputParams.length; chunkIndex++) {
        if (batch) {
            const tx = await configurator.populateTransaction.initReserves(chunkedInitInputParams[chunkIndex], { gasLimit: 10000000 });
...
const configureReservesByHelper = async (reservesParams, tokenAddresses, batch = false) => {
    ...
    if (tokens.length) {
        const aclAdmin = await hre.ethers.getSigner(await addressProvider.getACLAdmin());
        {
            ...
            if (reservesSetupHelperOwner !== aclAdmin.address) {
                console.log("Transferring ownership of ReservesSetupHelper to ACL admin");
                await (0, tx_1.waitForTx)(await reservesSetupHelper
        ...
        if (!(await aclManager.isRiskAdmin(reservesSetupHelper.address)) &&
            constants_1.POOL_ADMIN[network].toLowerCase() ===
                reservesSetupHelperOwner.toLowerCase()) {
            console.log("Adding ReservesSetupHelper to risk admins");
            if (batch) {
                const tx = await aclManager.populateTransaction.addRiskAdmin(reservesSetupHelper.address);
                (0, transaction_batch_1.addTransaction)(tx);
            }
            else {
        ...
        console.log(`- Configure reserves in ${chunkedInputParams.length} txs ${chunkedSymbols}`);
        for (let chunkIndex = 0; chunkIndex < chunkedInputParams.length; chunkIndex++) {
            if (batch) {
                const tx = await reservesSetupHelper.populateTransaction.configureReserves(poolConfiguratorAddress, chunkedInputParams[chunkIndex], { gasLimit: 1000000 });
                (0, transaction_batch_1.addTransaction)(tx);
            }
            else {
                const tx = await (0, tx_1.waitForTx)(await reservesSetupHelper.configureReserves(poolConfiguratorAddress, chunkedInputParams[chunkIndex], { gasLimit: 1000000 }));
