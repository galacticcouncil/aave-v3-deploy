...
const base_1 = __importDefault(require("../markets/base"));
const utils_2 = require("./utilities/utils");
const deploy_ids_1 = require("./deploy-ids");
const constants_1 = require("./constants");
const _1 = require(".");
const env_1 = require("./env");
const hydration_1 = __importDefault(require("../markets/hydration"));
var ConfigNames;
(function (ConfigNames) {
    ...
    ConfigNames["Base"] = "Base";
    ConfigNames["baseGoerli"] = "base-goerli";
    ConfigNames["Hydration"] = "Hydration";
...
const savePoolTokens = async (reservesConfig, dataProviderAddress) => {
    ...
    return bluebird_1.default.each(Object.keys(reservesConfig), async (tokenSymbol) => {
        ...
        const aToken = `${tokenSymbol}${deploy_ids_1.ATOKEN_PREFIX}`;
        const variableDebtToken = `${tokenSymbol}${deploy_ids_1.VARIABLE_DEBT_PREFIX}`;
        const stableDebtToken = `${tokenSymbol}${deploy_ids_1.STABLE_DEBT_PREFIX}`;
        if ((await hre.deployments.getOrNull(aToken))?.address === constants_1.ZERO_ADDRESS) {
            await hre.deployments.save(aToken, {
                address: aTokenAddress,
                ...aTokenArtifact,
            });
        }
        if ((await hre.deployments.getOrNull(variableDebtToken))?.address ===
        ...
        if ((await hre.deployments.getOrNull(stableDebtToken))?.address ===
            constants_1.ZERO_ADDRESS) {
...
const getReserveAddresses = async (poolConfig, network) => {
    ...
    const testnetTokenKeys = Object.keys(allDeployments).filter((key) => key.includes(deploy_ids_1.TESTNET_TOKEN_PREFIX) &&
        reservesKeys.includes(key.replace(deploy_ids_1.TESTNET_TOKEN_PREFIX, "")));
    return testnetTokenKeys.reduce((acc, key) => {
        const symbol = key.replace(deploy_ids_1.TESTNET_TOKEN_PREFIX, "");
