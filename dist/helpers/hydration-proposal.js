...
async function generateProposal(transactions, from, registerAssets = [], whitelist = false) {
    ...
    const evmCall = ({ from, to, data, gas = "100000", gasPrice = "600000000", }) => evm.call(evmAddress(from), to, data, "0", gas, gasPrice, undefined, undefined, []);
    const rootEvmCall = ({ from, to, data, gas = "100000", gasPrice = "600000000", }) => utility.dispatchAs({ system: { signed: from } }, evmCall({ from, to, data, gas, gasPrice }));
    const registerAsset = ({ asset, address, symbol, decimals }) => assetRegistry.register(asset, symbol, "Erc20", 0, symbol, decimals, location(address), null, true);
    const batch = [
        ...transactions.map((tx) => rootEvmCall({
            ...tx,
    ...
    if (whitelist) {
        ...
        const whitelist = api.tx.whitelist.whitelistCall(whitelistedCall.hash).method;
        const proposal = api.tx.whitelist.dispatchWhitelistedCallWithPreimage(whitelistedCall).method;
