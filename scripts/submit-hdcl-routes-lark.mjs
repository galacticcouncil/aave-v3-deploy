...
// for the new HDCL stablepool, signed as //Alice on lark.
//
//   H2O (1)   → HDCL (550): [Omnipool 1→222, Stableswap 10055: 222→550]
//   WETH (20) → HDCL (550): [Stableswap 104: 20→1007, Stableswap 4200: 1007→4200,
...
for (const [ai, ao, label] of [[H2O, HDCL, "H2O→HDCL"], [WETH, HDCL, "WETH→HDCL"]]) {
...
const route_H2O_HDCL = api.tx.router.forceInsertRoute(
  { assetIn: H2O, assetOut: HDCL },
  [
    { pool: "Omnipool", assetIn: H2O, assetOut: HOLLAR },
    { pool: { Stableswap: POOL_ID }, assetIn: HOLLAR, assetOut: HDCL },
  ],
);

const route_WETH_HDCL = api.tx.router.forceInsertRoute(
  { assetIn: WETH, assetOut: HDCL },
  [
...
const batchAll = api.tx.utility.batchAll([route_H2O_HDCL, route_WETH_HDCL]);
