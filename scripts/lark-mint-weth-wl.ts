...
// breaks `lark-fund-account.ts` when funding the deployer's EVM-truncated account
// (deployer needs WETH for EVM gas). This script uses the same governance pattern
// as `lark-runtime-upgrade-wl.ts` to inject WETH directly via `tokens.setBalance`
...
//
// After this succeeds, run the standard `lark-fund-account.ts` to transfer HDX
