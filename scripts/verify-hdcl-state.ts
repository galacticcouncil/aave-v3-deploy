// Post-proposal state check against a forked HDCL market on chopsticks/lark2.
// Run via: HARDHAT_NETWORK=lark2 RPC=ws://localhost:8000 \
//   npx hardhat run scripts/verify-hdcl-state.ts --network lark2
//
...
async function main() {
  ...
  const dataProvider = await eth.getContractAt(
    ...
    (await hhre.deployments.get("PoolDataProvider-HDCL")).address
  );
  const acl = await eth.getContractAt(
    ...
    (await hhre.deployments.get("ACLManager-HDCL")).address
  );
  ...
  console.log(`  DCL    initialized: ${hasDcl ? "✓" : "✗"}`);
  ...
  // PoolAddressesProviderRegistry — confirm HDCL is registered.
  ...
  console.log(`\nProviderRegistry:`);
  console.log(`  HDCL provider ${provider} -> id ${fmt(id)} (want 22222255)`);
  ...
  for (const [id, label] of [[550, "DCL"], [55, "HDCL"]] as [number, string][]) {
  ...
  // evmAccounts.approveContract for Pool-Proxy-HDCL
  const poolProxyAddr = (await hhre.deployments.get("Pool-Proxy-HDCL")).address;
